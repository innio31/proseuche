    ...
    <div class="screen stack">
      ...
      ${!isStandalone() ? `
        <div class="card">
          ...
          ${isIOS()
            ? '<p>Tap the Share icon in Safari, then "Add to Home Screen".</p>'
            : `<p>Install Proseuche on this device for quick, full-screen access.</p><button class="btn btn-secondary" id="install-app" ${canPromptInstall() ? '' : 'disabled'}>${canPromptInstall() ? 'Install app' : 'Not available yet — keep browsing, then try again'}</button>`}
        </div>
      ` : ''}
